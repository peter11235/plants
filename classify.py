...
ap.add_argument("-m", "--masks", required = False,
...
model = RandomForestClassifier(n_estimators = 25, random_state = 100)
...
# Instead of grabbing random images, we are testing the images we got (they are
# all at the end of the list)
for i in range(len(imagePaths)-60,len(imagePaths)):
    imagePath = imagePaths[i]
    image = cv2.imread(imagePath)
    img_mask = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # describe the image
    features = desc.describe(image, img_mask)
